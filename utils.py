...
def mkv_to_mp4(mkv_path, remove_mkv=False):
    ...
    # cmd = 'ffmpeg -i {} -c:v copy -c:a copy {}'.format(mkv_path, mp4_path)
...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    # out_path = join_dir(out_dir, 'epoch{:0>2}_human_machine_pilot75.mkv'.format(epoch_id))
    # out_path = join_dir(out_dir, 'epoch{:0>2}_human_machine_vgg.mkv'.format(epoch_id))
    out_path = join_dir(out_dir, 'epoch{:0>2}_0708.mkv'.format(epoch_id))
...
def get_model():
    ...
    """
    # model_path = join_dir(params.model_dir, 'model_pilot75.json')
    # param_path = join_dir(params.model_dir, 'model_pilot75.h5')

    # model_path = join_dir(params.model_dir, 'model_vgg.json')
    # param_path = join_dir(params.model_dir, 'model_vgg.h5')

    model_path = join_dir(params.model_dir, 'model_0708.json')
    param_path = join_dir(params.model_dir, 'model_0708.h5')
